...
func TestServiceCreateOnServerReturnsAgent(t *testing.T) {
	...
	serverID := uuid.New()
	require.NoError(t, db.Create(&server2.Server{
		ID:       serverID,
...
func TestServiceCreateOnServerReturnsServerNotFound(t *testing.T) {
	svc := NewService(stubAgentRepository{}, stubServerLookup{err: server2.ErrServerNotFound}, testutil.NewPostgresDB(t), nil)

	name := "datadog"
	_, err := svc.CreateOnServer(context.Background(), uuid.New().String(), api.ServerAgentRequest{Name: &name})
	require.ErrorIs(t, err, server2.ErrServerNotFound)
}
...
var _ server2.LookupService = stubServerLookup{}
