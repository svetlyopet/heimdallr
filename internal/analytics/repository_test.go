...
func TestRepositoryGetAutomationOverviewAggregatesData(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, int64(2), response.TotalAutomations)
	require.Equal(t, int64(3), response.TotalJobs)
	require.Equal(t, int64(1), response.SuccessfulJobs)
	require.Equal(t, int64(1), response.FailedJobs)
	require.Equal(t, int64(1), response.StartedJobs)
