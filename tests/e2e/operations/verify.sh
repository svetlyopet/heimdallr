...
success_json="$(curl -sf "${auth[@]}" "${HEIMDALLR_URL}/api/v1/automation/${AUTOMATION_ID}/job/${JOB_ID_SUCCESS}")"
failure_json="$(curl -sf "${auth[@]}" "${HEIMDALLR_URL}/api/v1/automation/${AUTOMATION_ID}/job/${JOB_ID_FAILURE}")"
