...
import (
	...
	job2 "github.com/svetlyopet/heimdallr/modules/job"
...
func TestRepositoryCreateAndFindById(t *testing.T) {
	db := newJobTestDB(t)
	repo := job2.NewRepository(db)

	fixture := testfixtures.SeedProviderAutomation(t, db, "awx", "deploy")

	created, err := repo.Create(context.Background(), job2.Job{
		ID:           "1000",
...
func TestRepositoryUpdateChangesStatus(t *testing.T) {
	...
	updated, err := repo.Update(context.Background(), job2.Job{
		ID:           created.ID,
		AutomationID: fixture.Automation.ID,
		Status:       "success",
...
func TestRepositoryFindByIdReturnsNotFoundForDeletedAutomation(t *testing.T) {
	...
	var rawCount int64
	require.NoError(t, db.Unscoped().Model(&job2.Job{}).Where("id = ?", created.ID).Count(&rawCount).Error)
