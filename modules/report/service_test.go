...
import (
	...
	"github.com/svetlyopet/heimdallr/modules/report/api"
)

func newReportService(t *testing.T) (Service, application2.Service, release2.Service) {
	t.Helper()

	db := testutil.NewPostgresDB(t)
	appRepo := application2.NewRepository(db)
	appSvc := application2.NewService(appRepo, nil)
	releaseRepo := release2.NewRepository(db)
	releaseSvc := release2.NewService(releaseRepo, appSvc, nil)
